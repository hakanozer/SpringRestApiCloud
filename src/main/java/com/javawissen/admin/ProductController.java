...
public class ProductController {
	...
	public String ajaxProductPage(@RequestParam int itemCount) {
		...
		@SuppressWarnings("unchecked")
		List<Products> ls = sesi.createQuery("from Products order by productid desc").setFirstResult((itemCount -1)  * 10)
	...
	// ajaxPageCount
		...
		// Arama Ýþlemi
				...
				public String ajaxProductSearch(@RequestParam String ara,@RequestParam int itemCount) {
					...
					ara="%"+ara+"%";
      				@SuppressWarnings("unchecked")
					List<Products> ls = sesi.createQuery("from Products  where producttitle like '"+ara+"' or productdescription like '"+ara+"'").setFirstResult((itemCount -1)  * 10).setMaxResults(10).list();
					boyut1=ls.size();
					for (Products item : ls) {
						String rw = "<tr  role=\"sil\" id=\""+item.getProductid()+"\">\r\n" + 
								"				<td>"+item.getProductid()+"</td>\r\n" + 
								...
								"			<td>\r\n" + 
								"				<button onclick=\"fncDelete("+item.getProductid()+", 'sid' ,'sample')\" type=\"button\" class=\"btn btn-danger btn-sm\">Sil</button>\r\n" + 
								"				<button type=\"button\" class=\"btn btn-primary btn-sm\">Düzenle</button>\r\n" + 
								"				</td>\r\n" + 
								"			</tr>";
						bl.append(rw);
				...
				// Arama Ýþlemi ajaxPageCount
					...
					public String ajaxProductPageCountSearch(@RequestParam String ara) {
						...
						Session sesi = sf.openSession();
						@SuppressWarnings("unchecked")
						List<Sample> ls = sesi.createQuery("from Products  where producttitle like '"+ara+"' or productdescription like '"+ara+"'").list();
						int sayi = ls.size();
						int boyut=0;
						if(sayi%10==0)
						{
					...
					public String productInsert(Model model,Products prd) {
						Session sesi = sf.openSession();
